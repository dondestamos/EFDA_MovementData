classdef fdawarp
    ...
    % Updated   : A.D. Krotov (AKR) <krotov.a@northeastern.edu>
        % Focused on extending the method "time_warping".
        % Added calculation of elastic distances from signals to the mean, computing them
        % as L2-norm in signal or warp space. Just another proxy of variability
        % For alignment iterations, added history of iterations as sub-fields of the class object
    % Date      : 31-Apr-2023
    ...
    properties
        ...
        %%% AKR added
        TimeEl % Time elapsed
        FullDist % Amp distances to the mean, defined in terms of original f
        ElasticAmpDist % Elastic amp distances to the mean, defined via SRVF of f
        ElasticPhDist % Elastic phase distances to the mean, defined via SRVF of gammas
        IterationHistory_flag % Enable or disable logging previous iterations (enabling increases computation time by 1.5-2 times);
        Iterations % subfield with the same structure as Obj, but for every iteration.
    ...
    methods
        ...
        function obj = time_warping(obj,lambda,option)
            ...
            if nargin < 3
                ...
                option.MaxItr = 20;
                option.Quiet = 0;
                option.IterationHistory_flag = 0;
            end
            if nargin < 2
                lambda = 0;
            end
            ...
            if ~option.Quiet
                if lambda < 1e-3
                    fprintf('Computing Karcher mean and warps of %d functions in SRVF space via %s at λ=%.2g...\n',N,option.method,lambda);
                else
                    fprintf('Computing Karcher mean and warps of %d functions in SRVF space via %s at λ=%.3f...\n',N,option.method,lambda);
                end
                %fprintf('\n lambda = %5.3f', lambda);
            end
            ds = inf;
            ...
            for r = 1:MaxItr
                ...
                % AK: slicing variables to reduce memory usage.
                mqp = mq(:,r);
                timep = obj.time;
                mfp = mf(1,r);
                f1p = f1(:,:,1);
                methodp = option.method;
                wp = option.w;
                splp = option.spl;
                if option.parallel == 1
                    parfor k = 1:N
                        q_c = q(:,k,1); mq_c = mqp;
                        gam_o(k,:) = optimum_reparam(mq_c,q_c,timep,lambda,methodp,wp, ...
                            mfp, f1(1,k,1));
                        gam_dev(k,:) = gradient(gam_o(k,:), 1/(M-1));
                        f_temp(:,k) = warp_f_gamma(f1p(:,k),gam_o(k,:),timep);
                        q_temp(:,k) = f_to_srvf(f_temp(:,k),timep,splp);
                    end
                else
                    for k = 1:N
                        q_c = q(:,k,1); mq_c = mqp;
                ...
                % AKR: Something for debugging, not called anywhere else. Seemingly supposed 
                % to decrease from +inf to something finite, probably asymptomatically
                % Seemingly a cost function of total deviations, plus smoothness penalty
                ...
                qun_o(r) = norm(mq(:,r+1)-mq(:,r))/norm(mq(:,r)); % Scaled difference between 2-norms of means at current and previous iterations
                
                % AKR: adding two more sections here as a function, to output and illustrate the
                % iterative improvement. It generally appears that most of alignment is
                % well-done on the first iteration. And after 2-3 iterations, local-minima
                % oscillations begin, not improving the alignment.
                objS = struct;
                obj.Iterations = [];
                if option.IterationHistory_flag
                    objS = IterativeEvaluation(obj,r,option,N,M,q,mq,lambda,mf,f1,gam_o,binsize,qun_o,timeElstart);

                    fldns = fieldnames(objS);
                    for ifldn = 1:length(fldns)
                        fldn = fldns{ifldn};
                        obj.Iterations(r).(fldn) = objS.(fldn);
                    end
                end
            ...
            var_fgam = var(fgam,[],2); % DOUBLECHECK THIS


            obj.stats.orig_var = trapz(obj.time,std_f0.^2); % Will be normed to average duration of original signals
            obj.stats.amp_var = trapz(obj.time,std_fn.^2); % Will be normed to average duration of original signals
            obj.stats.phase_var = trapz(obj.time,var_fgam); % Will be normed to average duration of original signals
            ...
            % AKR: Have an additional cost function: These elastic distances. Their STD and mean. 
            % I think the original cost-fn is somewhat similar to the d/dIter(meanAmpDist). 
            % Maybe it is a way to make everything faster???...
            
            % AKR: Add average timeshift and timespeed too to further specify temporal
            % variation - or better in a wrapper.
            
            % AKR: Add elastic distances here.
            ...
            %AKR: elastic distances to the mean + full distance (of original function)
            for ii = 1:N
                obj.FullDist(ii) = sqrt(trapz(obj.time,(obj.fn(:,ii) - obj.fmean).^2));
            ...
            % AKR
            % Do distances to mean remain invariant if the whole ensemble is warped?
            % Use an arbitrary warping function to warp all
            if 0
                
                [~, gamm, ~] = genwarp(0.8,0.5,obj.time,'InverseFlag',1,'Shift',pi/2);
                for ii = 1:N
                    fwarped(:,ii) = warp_f_gamma(obj.fn(:,ii),gamm,obj.time);
                    qwarped(:,ii) = warp_q_gamma(obj.qn(:,ii),gamm,obj.time);
                end
                fmean = mean(fwarped,2);
                qmean = mean(qwarped,2);
                for ii = 1:N
                    ElasticAmpDist2(ii) = sqrt(trapz(obj.time,(qmean - qwarped(:,ii)).^2));
                    ElasticPhDist2(ii) = acos(trapz(obj.time,obj.psi(:,ii)));
                end
                % The phase distances remain the same. The amplitude distances change but very little (maybe due to nonzero lambda). 
            end
            ...
            timeElfinish = toc;
            obj.TimeEl = timeElfinish - timeElstart;
        end
...
%##########################################################################
% Evaluate performance and output results at every iteration / AKR
%##########################################################################
function objS = IterativeEvaluation(obj,r,option,N,M,q,mq,lambda,mf,f1,gam_o,binsize,qun_o,timeElstart)
% last step with centering of gam
r = r+1;
...
obj.qn = q(:,:,r+1);
obj.q0 = q(:,:,1);
std_f0 = std(f1(:,:,1), 0, 2); % DOUBLECHECK THIS
std_fn = std(obj.fn, 0, 2); % DOUBLECHECK THIS
obj.mqn = mq(:,r+1);
obj.fmean = mean(obj.f(1,:))+cumtrapz(obj.time,obj.mqn.*abs(obj.mqn));
...
obj.stats.orig_var = trapz(obj.time,std_f0.^2); % DOUBLECHECK THIS
obj.stats.amp_var = trapz(obj.time,std_fn.^2); % DOUBLECHECK THIS
obj.stats.phase_var = trapz(obj.time,var_fgam); % DOUBLECHECK THIS
...
% AK: Have an additional cost function: These elastic distances. Their STD and mean.
% I think the original cost-fn is somewhat similar to the d/dIter(meanAmpDist).
% Maybe it is a way to make everything faster???...

% AK: Add average timeshift and timespeed too to further specify temporal
% variation - or better in a wrapper.

% AK: Add elastic distances here.
%             q1 = f_to_srvf(f1,time);
%             q2 = f_to_srvf(f2,time);
%             gam = optimum_reparam(q1,q2,time,lambda,method);
%             fw = warp_f_gamma(f2,gam,time.');
...
%AK: elastic distances to the mean + full distance (of original function)
for ii = 1:N
    obj.FullDist(ii) = sqrt(trapz(obj.time,(obj.fn(:,ii) - obj.fmean).^2));
    obj.ElasticAmpDist(ii) = sqrt(trapz(obj.time,(obj.mqn - obj.qn(:,ii)).^2));
    obj.ElasticPhDist(ii) = acos(trapz(obj.time,obj.psi(:,ii)));
...
%% Convert fdawarp object to regular matlab structure
objS = struct;
fldns = fieldnames(obj);
for ifldn = 1:length(fldns)
    fldn = fldns{ifldn};
    objS.(fldn) = obj.(fldn);
end
