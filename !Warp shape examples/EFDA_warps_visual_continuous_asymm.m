...
qsource = f_to_srvf(fsource,time');

%% Warping functions

% completely no inflection
i = 1; %convex first
[psi(:,i), gamm(:,i), shvec(:,i)] = genwarp(0.3,0.5,time,'InverseFlag',1,'Shift',pi/2);

i = 2; %concave first
[psi(:,i), gamm(:,i), shvec(:,i)] = genwarp(0.3,0.5,time,'InverseFlag',0,'Shift',pi/2);

% edge inflexion but still generally monotonic
i = 3;
[psi(:,i), gamm(:,i), shvec(:,i)] = genwarp(0.3,1,time,'InverseFlag',1,'Shift',0);

i = 4;
[psi(:,i), gamm(:,i), shvec(:,i)] = genwarp(0.3,1,time,'InverseFlag',0,'Shift',0);

% speedup/slowdown only, midpoint fixed
i = 5;
[psi(:,i), gamm(:,i), shvec(:,i)] = genwarp(0.3,2,time,'InverseFlag',1,'Shift',0);

i = 6;
[psi(:,i), gamm(:,i), shvec(:,i)] = genwarp(0.3,2,time,'InverseFlag',0,'Shift',0);

% speedup/slowdown only, 0.33 and 0.67 fixed
i = 7;
[psi(:,i), gamm(:,i), shvec(:,i)] = genwarp(0.3,3,time,'InverseFlag',1,'Shift',0);

i = 8;
[psi(:,i), gamm(:,i), shvec(:,i)] = genwarp(0.3,3,time,'InverseFlag',0,'Shift',0);
...
for i = 1:size(gamm,2)
    ...
    qwarped(:,i) = warp_q_gamma(qsource,gamm(:,i),time);
    % Norm preservving?
...
figname = 'EFDA warpings demo continuous asymmetric';
...
disp(sprintf('Original L2 norm %.2f  ',L2norm(fsource)));

disp('Warped L2 norms');
%fprintf('L2 norms');
for i = 1:size(gamm,2)
...
disp('');
disp(sprintf('Original Area %.2f  ',trapz(time,fsource)));
for i = 1:size(gamm,2)
    fprintf('%.2f  ',trapz(time,fwarped(:,i)));
end  
disp('');
