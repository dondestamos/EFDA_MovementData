...
% Find direction
mnpsi = mean(psi);
dqq = sqrt(sum((psi' - mnpsi'*ones(1,n)).^2,1)); % SR of SS differences from the mean
[~, min_ind] = min(dqq); 
mu = psi(min_ind,:); % The sample-psi 'closest to the mean'

%%% WHAT IF I TAKE MEAN PSI INSTEAD OF PSI(MIN_IND)?? Seems to result in the same
%%% gam_mu... Even in less iterations. But I'll abstain from keeping this change.
%mu = mnpsi;
